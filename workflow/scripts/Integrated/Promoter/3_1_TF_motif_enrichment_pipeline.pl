...
my ($motifdir, $outputdir, $tfmapfile, $motifmapfile, $geneoutput, $groupfile, $bwfile, $outpic, $beddir, $groupname, $help);
...
exit 1 unless $motifdir && $outputdir && $groupfile && $bwfile && $beddir;

my %tf_files = (
    'hs'   => "workflow/resources/Motif_tf_anno/Homo_sapiens_TF.txt",
...
if (!-e $tfmapfile) {
    die "TF map file does not exist: $tfmapfile";
}

my @types = ("up", "down");
$outpic //= $outputdir;
mkpath($outpic, 0, 0755) or die "Failed to create directory $outpic: $!" unless -e $outpic;
...
foreach my $type (@types) {
        ...
        unless (-e $motif_result_dir) {
            mkpath($motif_result_dir, 0, 0755) or die "Failed to create directory $motif_result_dir: $!";
        }
        foreach my $posfile (@posfiles) {
            my ($group) = $posfile =~ m{/([^/]+)_common_${type}_peaks\.bed$};
            $group //= "UnknownGroup";
            
            if (defined $groupname && $group ne $groupname) {
                next;
            }
            
            my $prefix = "${group}_Promoter_both_${type}_genes";
            my $fastafile = "$outputdir/${prefix}_target_seqs.fasta";
            my $motif_result_dir_group = "${motif_result_dir}";
            unless (-e $motif_result_dir_group) {
                mkpath($motif_result_dir_group, 0, 0755) or die "Failed to create $motif_result_dir_group: $!";
            }
            
            extract_fasta_by_genome($groupfile, $posfile, $fastafile);
            motif_enrichment($fastafile, $motif_result_dir_group, $motifdir);
            my $networkfile = "$outputdir/${group}_TF_motif_network_${type}.txt";
            build_motif_network_with_significance($motif_result_dir_group, $networkfile);
            if ($tfmapfile && $motifmapfile) {
                my $geneout = $geneoutput // "$outputdir/${group}_TF_gene_mapping_${type}.txt";
...
sub extract_fasta_by_genome {
    ...
    my $genome = $group_config->{species} // "mm";
    my %genome_files = (
        'mm'   => "workflow/resources/anno/RNAseq_anno/GRCm38/GRCm38.primary_assembly.genome.fa",
        'homo' => "workflow/resources/anno/RNAseq_anno/GRCh38/GRCh38.primary_assembly.genome.fa",
        'hs'   => "workflow/resources/anno/RNAseq_anno/GRCh38/GRCh38.primary_assembly.genome.fa",
    );
    exit 1 unless exists $genome_files{$genome};
    my $genomefile = $genome_files{$genome};
    if (!-e $genomefile) {
        die "Genome file does not exist: $genomefile";
    }
    
    # Check if genome is gzipped or not
    my $fh;
    if ($genomefile =~ /\.gz$/) {
        open($fh, "gzip -dc $genomefile|") or exit 1;
    } else {
        open($fh, "<", $genomefile) or exit 1;
    }
    ...
    while (<$fh>) {
        ...
        if (/^>(\S+)/) {
            ...
            $chromosome{$chromname} .= $_ if defined $chromname;
        }
    }
    close($fh);
    my %posHash;
...
sub process_bam_bw_files {
    my ($groupfile, $bwdir, $posfile, $prefix, $outpic, $outputdir) = @_;
    my @bw_files = glob("$bwdir/*.bw");
    my $group_config = LoadFile($groupfile);
    
    my $all_bws = join(" ", @bw_files);
    return unless $all_bws;
    ...
    unless (-e $matrix_file) {
        my $computeMatrix_cmd = "computeMatrix reference-point -S $all_bws -R $posfile -out $matrix_file " .
                                "--referencePoint center -b 1000 -a 1000 --binSize 10 --skipZeros -p 4 " .
                                "--samplesLabel $samples_label --outFileNameMatrix $signal_file";
...
sub motif_enrichment {
    ...
    unless(-e $outputdir){
        mkpath($outputdir, 0, 0755) or die "Failed to create directory $outputdir: $!";
    }
    ...
    foreach my $motiffile (@motiffiles){
        $motiffile =~ /.*\/(.*)\.meme/;
        my $motifname = $1;
        my $outdir = "$outputdir/$motifname";
        unless(-e $outdir){
            mkpath($outdir, 0, 0755) or die "Failed to create directory $outdir: $!";
        }
...
sub build_motif_network_with_significance {
    ...
    my @motifseqfiles = `find $outputdir -type f -name "sequences.tsv"`;
    chomp @motifseqfiles;
