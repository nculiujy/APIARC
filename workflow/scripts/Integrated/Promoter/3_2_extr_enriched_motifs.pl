...
if (!$tfmapfile) {
        ...
        die "Unknown genome: $genome" unless exists $tf_files{$genome};
    ...
    my %tfmapHash;
    open(MAP, "<$tfmapfile") or die "Cannot open $tfmapfile: $!";
while (<MAP>) {
    chomp;
    if (/^Mus_musculus/) {
...
foreach my $enfile (@enfiles) {
    chomp $enfile;
    next if $enfile =~ /^\s*$/;
    open(EN, "<$enfile") or die "Cannot open $enfile for reading: $!";
    while (<EN>) {
        chomp;
        next if /^rank/;
        my @fieldValues = split /\t/;
