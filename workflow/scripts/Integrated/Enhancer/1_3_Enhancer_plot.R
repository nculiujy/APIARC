...
chip_csv_files <- file.path(chip_dirs, paste0(basename(chip_dirs), "_peak_anno.csv"))
...
if (length(chip_list) == 0) {
    cat("No ChIPseq enhancer peaks found.\n")
    q()
}
...
true_enh <- read.table(enhancer_bed_file, header = FALSE, stringsAsFactors = FALSE)
if (ncol(true_enh) >= 5) {
    colnames(true_enh)[1:5] <- c("chr", "start", "end", "name", "SYMBOL")
    true_enh$matrix_id <- paste0(true_enh$chr, ":", true_enh$start, "-", true_enh$end)
} else {
    cat("Error: enhancer_bed_file does not have 5 columns for SYMBOL matching.\n")
    q()
}


matrix_data <- read.table(matrix_file, skip = 3, header = FALSE, stringsAsFactors = FALSE, fill = TRUE, sep="\t")


if (nrow(matrix_data) != nrow(true_enh)) {
    cat(sprintf("Error: matrix rows (%d) != true_enh rows (%d)\n", nrow(matrix_data), nrow(true_enh)))
    q()
}


header_line <- strsplit(readLines(matrix_file, n = 3)[3], "\t")[[1]]

bin_samples <- header_line[-1]


bed_info <- true_enh[, c("chr", "start", "end", "name", "SYMBOL", "matrix_id")]

colnames(bed_info) <- c("V1", "V2", "V3", "V4", "V5", "V6")

signal_mat <- matrix_data
signal_mat[] <- lapply(signal_mat, function(x) as.numeric(as.character(x)))
signal_mat[is.na(signal_mat)] <- 0


if (length(bin_samples) != ncol(signal_mat)) {
    cat("Warning: length(bin_samples) != ncol(signal_mat), might truncate.\n")
    min_len <- min(length(bin_samples), ncol(signal_mat))
    bin_samples <- bin_samples[1:min_len]
    signal_mat <- signal_mat[, 1:min_len, drop=FALSE]
}

unique_samples <- unique(bin_samples)
mean_mat <- data.frame(matrix(ncol = length(unique_samples), nrow = nrow(signal_mat)))
colnames(mean_mat) <- unique_samples

for (j in seq_along(unique_samples)) {
    samp <- unique_samples[j]
    sample_cols <- signal_mat[, bin_samples == samp, drop = FALSE]
    mean_mat[, j] <- rowMeans(sample_cols, na.rm = TRUE)
}

mean_matrix_list <- list()
current_group <- basename(dirname(output_folder))
mean_matrix_list[[current_group]] <- cbind(bed_info, mean_mat)
...
chip_all_symbols <- unique(true_enh$SYMBOL)
...
get_group_means <- function(df, group_name, input_pattern = "Input") {
  ...
  tmp <- merge(tmp, true_enh[, c("chr", "start", "end", "SYMBOL")], by=c("chr", "start", "end"), all.x=TRUE)
...
write.csv(Result_all, file.path(output_folder, "Enhancer_gene_all.csv"), row.names = FALSE)
...
for (grp in valid_groups) {
  ...
  venn_up <- venn.diagram(
    ...
    cat.fontfamily = "sans",
    scaled = FALSE,
    main = paste(grp, "Common Up-regulated Genes in Enhancer Region: ", length(intersect(RNAseq_up, ChIPseq_up))),
    main.cex = 1.5,
    main.fontface = "bold",
  ...
  venn_down <- venn.diagram(
    ...
    cat.fontfamily = "sans",
    scaled = FALSE,
    main = paste(grp, "Common Down-regulated Genes in Enhancer Region: ", length(intersect(RNAseq_down, ChIPseq_down))),
    main.cex = 1.5,
    main.fontface = "bold",
    main.fontfamily = "sans",
    margin = 0.1,
